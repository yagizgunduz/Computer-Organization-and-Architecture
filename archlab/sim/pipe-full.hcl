...
# I improved IRET to reduce bubble for jumptable
# mrmovl JT(rA), rA + pushq rA + ret combination to simulate jmp *JT(rA))
# After pushq rA + ret, we use forward E_valA of pushq when the next instruction of ret is in Fetch stage.
# When combined with a mrmovq instruction, mrmovq JT(rA), rA + pushq rA would cause a load/use hazard.
# It should be solved by load forwarding, here due to the requirement of pushq + ret combination,
# I inserted a bubble into mrmovq + pushq.
# m_valM for the operands of IOPQ in Execute stage when a load/use hazard happens. -> avoid bubble
# Improved jXX instruction. (jle -> it is used before iaddq decision) 
# additional wordsig D_ifun for improveness
# some changes in ifun , getting new CC in Execute stage
...
wordsig D_ifun 'if_id_curr->ifun'     # Instruction function (added for ifun forwarding)
...
word f_pc = [
    ...
    D_icode == IJXX && !M_Cnd && !(E_icode in { IOPQ, IIADDQ }) && !(M_icode in { IOPQ, IIADDQ, IJXX, IRRMOVQ })
...
word f_ifun = [
        imem_error : FNONE;
        !(imem_icode in { IOPQ, IJXX, IRRMOVQ}) : 1;  # ifun of jle
        1: imem_ifun;
];
...
word e_valA = [
    E_srcA == M_dstM && M_icode in {IMRMOVQ, IPOPQ}
        && E_icode in {IRMMOVQ, IPUSHQ}: m_valM;
...
bool F_stall =
     ...
     ((E_dstM == d_srcA && !(D_icode in {IRMMOVQ, IOPQ}) && !(D_icode == IPUSHQ && f_icode != IRET))
     || (E_dstM == d_srcB && D_icode != IOPQ)) ||
        # Stalling at fetch while ret passes through pipeline
        ((D_icode == IRET && E_icode != IPUSHQ) ||
     (E_icode == IRET && M_icode != IPUSHQ) || (M_icode == IRET && W_icode != IPUSHQ));
...
bool D_bubble =
        # Mispredicted branch
        (D_icode == IJXX && !e_Cnd && !(E_icode in { IOPQ, IIADDQ }) && D_ifun == E_ifun &&
     !(!(M_icode in { IOPQ, IIADDQ }) && D_ifun == M_ifun)) ||
        (E_icode == IJXX && !e_Cnd && !(!(M_icode in { IOPQ, IIADDQ }) && E_ifun == M_ifun)
     && !(!(M_icode in { IOPQ, IIADDQ }) && !(W_icode in { IOPQ, IIADDQ, IJXX, IRRMOVQ }) && E_ifun == 1)) ||
...
bool E_stall = 0;
bool E_bubble =
        (E_icode == IJXX && !e_Cnd && !(!(M_icode in { IOPQ, IIADDQ}) && !(W_icode in { IOPQ, IIADDQ, IJXX, IRRMOVQ })
          && E_ifun == 1)) ||
        # Conditions for a load/use hazard
        E_icode in { IMRMOVQ, IPOPQ } &&
         ((E_dstM == d_srcA && !(D_icode in {IRMMOVQ, IPUSHQ, IOPQ})) || (E_dstM == d_srcB && D_icode != IOPQ));
